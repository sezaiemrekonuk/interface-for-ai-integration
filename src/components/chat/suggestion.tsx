...
function Suggestion({ text }: { text: string }) {
    ...
    return (
        <div className="flex grow rounded-lg p-2 lg:p-6 w-3/5 lg:w-1/3 min-h-[50px] max-h-[300px] bg-highlight flex-center bg-highlight-hover hover:cursor-pointer border dark:border-slate-600" onClick={() => setPlaceholder(text)}>
            <p className="">{text}</p>
        </div>
    )
}